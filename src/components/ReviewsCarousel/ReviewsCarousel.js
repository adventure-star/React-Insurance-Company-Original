...
import CarouselCarddraft from './CarouselCardsdraft.js'
...
class ReviewsCarousel extends Component {
    ...
    handleNext = (i) => {
        ...
        this.setState({...this.state, currentCard: 0}, () => {
            this.cardContainer.style.transitionDuration = '0.8s';
            this.cardContainer.style.transform = `translate(-${(485 * i)}px)`})
        } else if(this.state.currentCard < this.cardContainer.children.length -1){
        let newCurrentCard = i
        this.setState({...this.state, currentCard: newCurrentCard}, () => {
            this.cardContainer.style.transitionDuration = '0.8s';
            this.cardContainer.style.transform = `translate(-${(432,469 * i)}px)`
    ...
    getReviews(){
        let items = Reviews
        return items.map ( (item, i) => <CarouselCarddraft key={i} id={i} item={item} currentCard={this.state.currentCard}/>
        )
    }
    renderPagination(){
        let items = Reviews
