...
export default class HomeFormSecond extends Component {
    ...
    componentDidMount() {
        ...
        .then(res => {
          showMessage("getBrands", res)
            this.setState({...this.state, brandOptions: res, brandReady: true})
        })
    }
    componentDidUpdate() {
        if (this.state.formCompleted) {
            ...
            this.isNew && validOption2(this.state.isNew, this.isNew, this.isNewError)
    ...
    handleClick(e) {
        ...
        // const currentPolicy = validOption2(this.state.currentPolicy, this.currentPolicy, this.currentPolicyError)
        const franchise = this.franchise ? validFranchise(this.state.franchise, this.franchiseError) : "0"

        if (this.isNew) {
            validOption2(this.state.isNew, this.isNew, this.isNewError)
        }
    ...
    render() {
        return (
            <form className="form-container">
                ...
                <div className="form-line">
                    <Autocomplete label="Versión" options={this.state.modelsOptions.map(item => item.name)} onChange={(e) => this.handleModel(e)} key="id" value="name"  loading={!this.state.modelReady}/>
